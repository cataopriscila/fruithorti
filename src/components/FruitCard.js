...
const FruitCard = ({ fruit, onRouteChange, addToCart }) => {
  ...
  useEffect(() => {
    if (fruit.nutritions) {
      setNutriFacts(
        Object.entries(fruit.nutritions).map((nutri) => nutri.join(": "))
      );
    }
  }, [fruit.nutritions]);
  ...
  useEffect(() => {
    if (fruit.name) {
      unsplash
        ...
        .then((response) => {
          setFruitDetails((prev) => ({ ...prev, ...response.data.results[0] }));
        });
    }
  }, [fruit.name]);
        ...
        <Grid item xs={12} sm={6} md={4}>
          ...
          >
            ...
            <CardContent sx={{ flexGrow: 1 }}>
              ...
              <Typography>{`USD ${(fruitDetails.width * 0.001).toFixed(
                2
              )} / kg`}</Typography>
            ...
            <CardActions>
              ...
              <Button
                onClick={() => onRouteChange("shoppingcart")}
                size="small"
              >
                Cart
              </Button>
